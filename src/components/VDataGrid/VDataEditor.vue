<template>
	...
	>
		<template v-slot="{ selection }">
			<VLabel
				class="text-theme-text w-full h-full outline outline-2 -outline-offset-1 outline-theme-bg-300 hover:outline-theme-bg-500 focus:outline-theme-secondary-500 px-2 overflow-hidden overflow-ellipsis whitespace-nowrap"
				>{{ selection }}</VLabel
			>
		</template>
	</VDropdown>
	<VTextField
		v-model="model"
		v-else
		custom-class="w-full h-full outline outline-2 -outline-offset-1 outline-theme-bg-300 hover:outline-theme-bg-500 focus:outline-theme-secondary-500 px-2"
		:id="id"
		auto-focus
	/>
</template>
