<template>
  <div ref="datePicker" class="relative">
    ...
    <Teleport :to="appendTo" :disabled="!appendTo">
      <div
        ref="calendar"
        class="absolute z-20 shadow-md shadow-color-bg-100 top-full flex flex-col gap-2 bg-color-bg border border-color-border rounded-lg p-4 w-max max-w-max text-center"
        v-show="open"
      >
...
import { ref, computed, RendererElement } from "vue";
...
onClickOutside(calendar, () => (open.value = false));
...
function toggle() {
  ...
  if (open.value) {
    requestAnimationFrame(() => {
      if (datePicker.value && calendar.value && props.appendTo) {
        useAutoPopDirection(datePicker.value, calendar.value);
      }
    });
  }
}
