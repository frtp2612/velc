<template>
  <div
    ref="dropdown"
    class="border border-slate-100 rounded cursor-pointer relative"
    @click="toggle"
  >
    <span class="px-4 py-2 h-6">{{ formattedSelectedItem }}</span>
    <Teleport to="body" :disabled="true">
      <slot :items="filteredItems">
        <div
          class="absolute z-50 top-full border border-slate-100 shadow-sm flex flex-col max-h-32 min-w-[16rem] w-max max-w-max bg-white"
          v-show="open"
        >
          <VirtualScroller :items="filteredItems" :item-height="30">
            <template v-slot="{ item }">
              <div
                class="px-2 hover:bg-blue-200 overflow-ellipsis whitespace-nowrap"
