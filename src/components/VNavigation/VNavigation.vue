<template>
	<div
		class="flex flex-col h-full min-h-0 gap-4 bg-color-nav p-4 border border-color-border-50 transition-all ease-out duration-200"
	...
	>
		...
		<VDivider />
		<VLabel
			class="text-color-text-500 font-bold text-sm transition-opacity ease-out"
			v-show="isExpanded"
			:class="[isExpanded ? 'opacity-100' : 'opacity-0']"
			>Menu</VLabel
		>
		<div
			class="overflow-y-auto overflow-x-hidden h-full flex flex-col gap-4 w-full"
		>
			<VNavigationNode
